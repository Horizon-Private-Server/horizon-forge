...
void collectibleCollect(Moby* moby)
{
  ...
  int hasCollected = (collectiblesMask & (1 << index)) != 0;

  if (!hasCollected) {

    // mark collected
    collectiblesMask |= (1 << index);
    ...
    // popup
    char strBuf[64];
    int count = countBits(collectiblesMask);
    snprintf(strBuf, sizeof(strBuf), "You found a Gold Bolt!\x01 Collected %d/%d", count, collectiblesCount);
