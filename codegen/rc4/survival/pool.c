...
void poolInitMoby(Moby *moby)
{
	...
	if (moby->JointCnt == 0)
		moby->ModeBits |= MOBY_MODE_BIT_UNK_40;
	if (moby->Shadow)
		moby->ModeBits |= MOBY_MODE_BIT_DRAW_SHADOW;
	if (moby->PUpdate == 0)
		moby->ModeBits |= MOBY_MODE_BIT_NO_UPDATE;
}
...
Moby *poolSpawn(int oclass, int pvarSize)
{
	...
	if (!moby || mobyIsDestroyed(moby))
	{
		...
		if (moby && mobyIsDestroyed(moby))
		{
			DPRINTF("POOL FOUND ALREADY DESTROYED MOBY %08X (%04X)\n", (u32)moby, moby->OClass);
		}
#endif
...
void poolDestroy(Moby *moby)
{
	...
	// remove from moby grid
	// UpdateMobyGrids()
	((void (*)(Moby *, u64))0x004fe440)(moby, 0x80807f7f);
}
