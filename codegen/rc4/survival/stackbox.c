...
{
	if (!moby)
		return StackboxItems[rand(StackboxItemsCount)];
...
void sboxPlayerBuy(Moby *moby, int playerId, enum StackableItemId item)
{
	...
	if (!MapConfig.State)
		return;
	if (MapConfig.State->PlayerStates[playerId].State.Bolts < cost)
		return;
	if ((gameGetTime() - pvars->ActivatedTime) < (TIME_SECOND * 0.2))
		return;
	if (!sboxCanBuy(playerId, item))
		return;
...
void sboxPlayerReroll(Moby *moby, int playerId, enum StackableItemId item)
{
	...
	if (MapConfig.State->PlayerStates[playerId].State.Bolts < cost)
		return;
	if ((gameGetTime() - pvars->ActivatedTime) < TIME_SECOND)
		return;
...
void sboxUpdate(Moby *moby)
{
	...
	if (MapConfig.State && MapConfig.State->RoundCompleteTime)
	{
		...
		{
			if (pvars->BaseMoby)
				pvars->BaseMoby->ModeBits = MOBY_MODE_BIT_HIDE_BACKFACES | MOBY_MODE_BIT_HAS_GLOW;
			mobySetState(moby, STACK_BOX_STATE_ACTIVE, -1);
			if (gameAmIHost())
				sboxActivate(moby, STACK_BOX_STATE_ACTIVE, sboxGetRandomItem(moby), gameGetTime());
		}
	}
	else
	{
		if (moby->State != STACK_BOX_STATE_DISABLED)
		{
			...
			if (gameAmIHost())
				sboxActivate(moby, STACK_BOX_STATE_DISABLED, sboxGetRandomItem(moby), gameGetTime());
		}
	}
#endif
	...
	for (i = 0; i < GAME_MAX_LOCALS; ++i)
	{
		...
		// prompt for
		if (!sboxCanBuy(player->PlayerId, pvars->Item))
		{
...
int sboxHandleEvent(Moby *moby, GuberEvent *event)
{
	...
	if (isInGame() && !mobyIsDestroyed(moby) && moby->OClass == STACK_BOX_OCLASS && moby->PVar)
	{
		...
		case STACK_BOX_EVENT_ACTIVATE:
			return sboxHandleEvent_Activate(moby, event);
		case STACK_BOX_EVENT_GIVE_PLAYER:
			return sboxHandleEvent_GivePlayer(moby, event);
		case STACK_BOX_EVENT_PLAYER_BUY:
			return sboxHandleEvent_PlayerBuy(moby, event);
		case STACK_BOX_EVENT_PLAYER_REROLL:
			return sboxHandleEvent_PlayerReroll(moby, event);
		default:
		{
		...
		if (gameIsStartMenuOpen(i) && !playerData->IsInWeaponsMenu)
		{
			...
			for (j = 0; j < StackboxItemsCount; ++j)
			{
				...
				case STACKABLE_ITEM_ALPHA_MOD_AREA:
					snprintf(buffer, 32, "+%d", count * ITEM_STACKABLE_ALPHA_MOD_AMT);
					break;
				case STACKABLE_ITEM_EXTRA_JUMP:
					snprintf(buffer, 32, "+%d", count);
					break;
				case STACKABLE_ITEM_EXTRA_SHOT:
					snprintf(buffer, 32, "+%d", count);
					break;
				case STACKABLE_ITEM_LOW_HEALTH_DMG_BUF:
					snprintf(buffer, 32, "+%.f%%", count * ITEM_STACKABLE_LOW_HEALTH_DMG_BUF_FAC * 100);
					break;
				case STACKABLE_ITEM_HOVERBOOTS:
					snprintf(buffer, 32, "+%.0f%%", ITEM_STACKABLE_HOVERBOOTS_SPEED_BUF * count * 100);
					break;
				case STACKABLE_ITEM_VAMPIRE:
					snprintf(buffer, 32, "+%d", ITEM_STACKABLE_VAMPIRE_HEALTH_AMT * count);
					break;
				case STACKABLE_ITEM_EXPLODING_ENEMIES:
					snprintf(buffer, 32, "+%d", ITEM_STACKABLE_EXPLODINGENEMIES_DAMAGE * count);
					break;
				}
