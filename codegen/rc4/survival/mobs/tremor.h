#ifndef SURVIVAL_MOB_TREMOR_H
#define SURVIVAL_MOB_TREMOR_H

#define TREMOR_BASE_REACTION_TICKS (0.25 * TPS)
#define TREMOR_BASE_ATTACK_COOLDOWN_TICKS (1 * TPS)
#define TREMOR_BASE_EXPLODE_RADIUS (5)
#define TREMOR_MELEE_HIT_RADIUS (1.0)
#define TREMOR_MELEE_ATTACK_RADIUS (5)

#define TREMOR_TARGET_KEEP_CURRENT_FACTOR (3)

#define TREMOR_MAX_WALKABLE_SLOPE (50 * MATH_DEG2RAD)
#define TREMOR_BASE_STEP_HEIGHT (3)
#define TREMOR_TURN_RADIANS_PER_SEC (90 * MATH_DEG2RAD)
#define TREMOR_TURN_LUNGE_RADIANS_PER_SEC (25 * MATH_DEG2RAD)
#define TREMOR_TURN_AIR_RADIANS_PER_SEC (15 * MATH_DEG2RAD)
#define TREMOR_MOVE_ACCELERATION (40)
#define TREMOR_MOVE_AIR_ACCELERATION (20)

#define TREMOR_DEFAULT_JUMP_SPEED (5)

// AnimSeqT range defines for animation state checks
#define TREMOR_ATTACK_HIT_FRAME_START (4)
#define TREMOR_ATTACK_HIT_FRAME_END (8)
#define TREMOR_ATTACK_ANIM_LUNGE_DURATION (10)
#define TREMOR_ATTACK_QUAKE_SPAWN_FRAME_START (12)
#define TREMOR_JUMP_ANIM_DURATION (21)
#define TREMOR_FLINCH_ANIM_AIR_DURATION (20)
#define TREMOR_FLINCH_ANIM_BACK_FLIP_FALL_DESTROY_FRAME (15)

#define TREMOR_QUAKE_DURATION_TICKS (1.5 * TPS)
#define TREMOR_QUAKE_HIT_RADIUS (1.25)
#define TREMOR_QUAKE_SPEED (30.0 * MATH_DT)
#define TREMOR_QUAKE_TURN_RADIANS_PER_SEC ((90.0 / 9.0) * MATH_DEG2RAD)
#define TREMOR_QUAKE_FIRE_AT_MAX_TURN_ANGLE (30 * MATH_DEG2RAD)

#define TREMOR_ANIM_ATTACK_TICKS (30)
#define TREMOR_ANIM_ATTACK_QUAKE_TICKS (3 * TPS)
#define TREMOR_FLINCH_COOLDOWN_TICKS (60 * 7)
#define TREMOR_KNOCKBACK_MULTIPLIER (1.5)
#define TREMOR_STATE_COOLDOWN_TICKS (30)
#define TREMOR_RESPAWN_AFTER_TICKS (60 * 30)
#define TREMOR_BASE_COLL_RADIUS (1.0)
#define TREMOR_MAX_COLL_RADIUS (4)
#define TREMOR_AMBSND_MIN_COOLDOWN_TICKS (60 * 2)
#define TREMOR_AMBSND_MAX_COOLDOWN_TICKS (60 * 3)
#define TREMOR_FLINCH_PROBABILITY (1.0)
#define TREMOR_FLINCH_PROBABILITY_PWR_FACTOR (0.1)

#define TREMOR_PRIMARY_COLOR (0x00464443)
#define TREMOR_GLOW_COLOR (0x80202020)
#define TREMOR_LOD_COLOR (0x0080FF80)

enum TremorAnimId
{
	TREMOR_ANIM_IDLE,
	TREMOR_ANIM_IDLE_CLEAN_SPIKE,
	TREMOR_ANIM_IDLE_LOOK_AROUND,
	TREMOR_ANIM_IDLE_READY,
	TREMOR_ANIM_LAUGHING,
	TREMOR_ANIM_RUN,
	TREMOR_ANIM_WALK,
	TREMOR_ANIM_JUMP,
	TREMOR_ANIM_SIDE_FLIP_RIGHT,
	TREMOR_ANIM_SIDE_FLIP_LEFT,
	TREMOR_ANIM_BACK_FLIP,
	TREMOR_ANIM_SWING,
	TREMOR_ANIM_JUMP_AND_STAB_DOWN,
	TREMOR_ANIM_STAB_DOWN_POSITION_REPEAT,
	TREMOR_ANIM_DANCE_SPIN_THING,
	TREMOR_ANIM_ATTACK_COMBO_2,
	TREMOR_ANIM_THROW,
	TREMOR_ANIM_FLINCH,
	TREMOR_ANIM_SPAWN,
	TREMOR_ANIM_FLINCH_FALL_GET_UP,
	TREMOR_ANIM_FLINCH_BACK_FLIP_FALL,
};

enum TremorSoundId
{
	TREMOR_SOUND_VOCAL_AND_SWING = 0,
	TREMOR_SOUND_SWING_1 = 1,
	TREMOR_SOUND_SPEAK_1 = 2,
	TREMOR_SOUND_STEP = 3,
	TREMOR_SOUND_GETHIT = 4,
	TREMOR_SOUND_SWING_2 = 7,
	TREMOR_SOUND_SPEAK_2 = 8,
	TREMOR_SOUND_GET_HIT_1 = 9,
	TREMOR_SOUND_GET_HIT_2 = 10,
	TREMOR_SOUND_GET_HIT_3 = 11,
	TREMOR_SOUND_DIE = 15,
};

enum TremorBangles
{
	TREMOR_BANGLE_HEAD = (1 << 0),
	TREMOR_BANGLE_HEAD_ARMOR = (1 << 1),
	TREMOR_BANGLE_CHEST = (1 << 2),
	TREMOR_BANGLE_CHEST_ARMOR = (1 << 3),
	TREMOR_BANGLE_LEFT_ARM = (1 << 4),
	TREMOR_BANGLE_LEFT_ARM_ARMOR = (1 << 5),
	TREMOR_BANGLE_HIDE_BODY = (1 << 15)
};

enum TremorStates
{
	TREMOR_STATE_SPAWN,
	TREMOR_STATE_IDLE,
	TREMOR_STATE_JUMP,
	TREMOR_STATE_WALK,
	TREMOR_STATE_FLINCH,
	TREMOR_STATE_BIG_FLINCH,
	TREMOR_STATE_LOOK_AT_TARGET,
	TREMOR_STATE_DIE,
	TREMOR_STATE_ATTACK,
	TREMOR_STATE_ATTACK_GROUND_QUAKE,
};

enum TremorSubskeletonJoints
{
	TREMOR_SUBSKELETON_JOINT_RIGHT_HAND_CLAW = 0,
	TREMOR_SUBSKELETON_JOINT_RIGHT_SHOULDER = 1,
	TREMOR_SUBSKELETON_JOINT_RIGHT_UPPER_LEG = 2,
	TREMOR_SUBSKELETON_JOINT_RIGHT_FOOT = 3,
};

enum TremorBehaviors
{
	TREMOR_BEHAVIOR_MELEE = 0,
	TREMOR_BEHAVIOR_RANGED = 1,
	TREMOR_BEHAVIOR_NORMAL = 2,
};

enum TremorActions
{
	TREMOR_ACTION_MELEE,
	TREMOR_ACTION_GROUND_QUAKE,
	TREMOR_ACTION_COUNT,
};

enum TremorParameters
{
	/* TREMOR_ACTION_MELEE */
	TREMOR_PARAM_MELEE_DAMAGE_MULTIPLIER = 0,
	TREMOR_PARAM_MELEE_ATTACK_SPEED_MULTIPLIER = 1,
	TREMOR_PARAM_MELEE_LUNGE_MULTIPLIER = 2,

	/* TREMOR_ACTION_GROUND_QUAKE */
	TREMOR_PARAM_GROUND_QUAKE_DAMAGE_MULTIPLIER = 3,
	TREMOR_PARAM_GROUND_QUAKE_QUAKE_SPEED = 4,
};

typedef struct TremorMobVars
{
	Moby *QuakeMoby;
	u32 ActionCooldownTicks[TREMOR_ACTION_COUNT];
	u32 ActionQueuedForTicks[TREMOR_ACTION_COUNT];
} TremorMobVars_t;

struct TremorQuakePVars
{
	char UNK_00[0x10];
	int bEnabled;
	int LifeTicks;
	char UNK_18[4];
	Moby *CreatorMob;
	char UNK_20[0x20];
};

struct MobConfig;

extern struct MobVTable TremorVTable;

#endif // SURVIVAL_MOB_TREMOR_H
