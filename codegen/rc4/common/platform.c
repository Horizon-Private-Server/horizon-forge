...
void platformFlipperDoFlip(Moby* moby)
{
  ...
  float flipAfterSec = moby->State == PLATFORM_FLIPPER_STATE_UP ? pvars->FlipAfterSeconds : (pvars->FlipAfterSeconds + PLATFORM_FLIP_SPEED + pvars->UnflipAfterSeconds);
  ...
  float secondsSinceLastFlip = fastmodf(t, pvars->FlipAfterSeconds + pvars->UnflipAfterSeconds + PLATFORM_FLIP_SPEED*2);
...
void platformFlipperDoFall(Moby* moby)
{
  ...
  if (moby->State == PLATFORM_FLIPPER_STATE_FALL) {
    ...
    } else if (secondsSinceLastFall < pvars->FallDelay) {
...
void platformFlipperUpdate(Moby* moby)
{
  ...
  if (moby->State == PLATFORM_FLIPPER_STATE_UP && pvars->FlipperRotation > 0) {
    ...
    moby->Rotation[0] = pvars->FlipperRotation * MATH_PI;
  } else if (moby->State == PLATFORM_FLIPPER_STATE_FALL && moby->Position[2] > 0 && pvars->FallAmount < absFallDist && secondsSinceLastStateChange > pvars->FallDelay) {
